  ...
  try {
    await Promise.all([
      resend.emails.send({
        ...
        subject: `Waitlist signup: AI Review Management — ${data.name}`,
        html: `
          <div style="font-family:sans-serif;max-width:600px;color:#1a1a1a;">
            <h2 style="color:#6366f1;margin-bottom:16px;">New Waitlist Signup</h2>
            <p style="font-size:14px;color:#555;margin-bottom:16px;"><strong>Service:</strong> AI Review Management</p>
      ...
      fetch(
        `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage`,
        {
          ...
          body: JSON.stringify({
            ...
            text: `New waitlist signup for AI Review Management: ${data.name} (${data.email})`,
          }),
        }
      ),
    ]);
